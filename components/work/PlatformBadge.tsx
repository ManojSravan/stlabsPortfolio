import type { SocialPlatform } from "@/lib/testimonials";
...
export function PlatformIcon({
  ...
  if (platform === "google") return <GoogleIcon className={className} />;
  return <LinkedInIcon className={className} />;
}

export function PlatformLabel({ platform }: { platform: SocialPlatform }) {
    ...
    >
      {Array.from({ length: 5 }).map((_, i) => (
        <svg
          key={i}
          viewBox="0 0 20 20"
          className={`h-3 w-3 ${i < rating ? "fill-current" : "fill-current opacity-25"}`}
          aria-hidden
        >
